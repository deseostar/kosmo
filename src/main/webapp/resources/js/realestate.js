				...
				function(){			
					if(memberId==this.memberId){ //글 작성자 == 댓글 작성자 - 글쓴이 태그
						...
						if(loginId==this.memberId){ // 댓글 작성자 == 로그인ID - 삭제 기능
							str +='<div class="btn-group"><button type="button" class="btn btn-default btn-sm dropdown-toggle myButton" id="toggle'+this.rCommentNumber+'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Delete<span class="caret"></span></button><ul class="dropdown-menu"><li onclick=deleteAccept('+this.rCommentNumber+')>&nbsp;Accept</li><li>&nbsp;Cancel</li></ul></div></span></div><div id="rCommentArea">'+this.rCommentContent+'</div></div></div></li></ol>';
						}else{
