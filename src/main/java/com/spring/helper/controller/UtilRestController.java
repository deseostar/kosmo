...
public class UtilRestController {
	...
	// 서울 버스 차량번호 구하기
		@RequestMapping(value="SeoulbusgetJson", method= RequestMethod.GET, produces = "text/plain;charset=UTF-8")
		public ResponseEntity<String> SeoulbusgetJson(HttpServletRequest req) throws Exception{
			logger.info("getBusRealTimeNo 로딩 중....");
			String route = req.getParameter("route");
			String url = "http://ws.bus.go.kr/api/rest/arrive/getArrInfoByRouteAll?serviceKey=GdoR86lqZXehbYz0fIJrJjrCLQq9UHQg9pk2RA8UgEhtJI8vJ45t8O%2B8p6N3QaDDUUkB1kUa1Ra%2BwQnLK%2FcHuQ%3D%3D&busRouteId="+route;
			RestTemplate restTemplate = new RestTemplate();
			URI uri = new URI(url);
			System.out.println("요청주소"+uri);
			String response = restTemplate.getForObject(uri, String.class);
